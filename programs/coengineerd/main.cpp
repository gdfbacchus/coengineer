...
int main(int argc, char** argv) {
   ...
   try {
...
#else
      std::cerr << "------------------------------------------------------\n\n";
      std::cerr << "            STARTING COENGINEER NETWORK\n\n";
      std::cerr << "------------------------------------------------------\n";
      std::cerr << "initminer public key: " << STEEMIT_INIT_PUBLIC_KEY_STR << "\n";
      std::cerr << "chain id: " << std::string(STEEMIT_CHAIN_ID) << "\n";
      std::cerr << "blockchain version: " << fc::string( STEEMIT_BLOCKCHAIN_VERSION ) << "\n";
      ...
      app_options.add_options()
            ...
            ("json,j", bpo::value<boost::filesystem::path>()->default_value("coengineer.json"), "JSON like accounts initialization")
      ...
      {
         auto json = options["json"].as<boost::filesystem::path>();
         if (boost::filesystem::exists(json) && boost::filesystem::is_regular_file(json))
         {
            auto snapshot = fc::json::from_file(json).as<steemit::chain::snapshot_state>();
            node->chain_database()->load_from_snapshot(snapshot);
         }
         else
         {
            std::cout << json << " is not a file\n";
         }
     }
