...
{
    auth_inhibitor( database& db) : db( db )
    {
        if (count()++ == 0)
        {
            old_flags = db.node_properties().skip_flags;
            db.node_properties().skip_flags |= database::skip_authority_check;
        }
    }
    auth_inhibitor( auth_inhibitor&& ) = delete;
    auth_inhibitor( const auth_inhibitor& ) = delete;
    auth_inhibitor& operator=( auth_inhibitor&& ) = delete;
    auth_inhibitor& operator=( const auth_inhibitor& ) = delete;
    ~auth_inhibitor()
    {
        if (--count() == 0)
        {
...
private:
    ...
    static size_t& count() { static size_t cnt = 0; return cnt; }
};

template<typename ObjectType, typename ObjectTag, typename Modifier>
void create_or_modify( database& db, const account_name_type& account_name, Modifier&& m )
{
    if( auto account = db.find< ObjectType, ObjectTag >( account_name ) )
    {
        db.modify( *account, std::forward< Modifier >( m ) );
    }
    else
    {
        db.create< ObjectType >( std::forward< Modifier >( m ) );
    }
}

void database::load_from_snapshot( const snapshot_state &snapshot )
{
    try
    {
        ...
        {
            create_or_modify< account_object, by_name >( *this, account.name, [&]( account_object& a )
            {
                a.name = account.name;
                a.memo_key = account.keys.memo_key;
                const auto &assets = account.balances.assets;
                if( assets.size() == 3 )
                {
                    a.balance = assets[0];
                    a.sbd_balance = assets[1];
                    a.vesting_shares = assets[2];
                }
                else
                {
                    std::cout << "Format differs assets: [balance, sbd_balance, vesting_shares]\n";
                }
                a.recovery_account = STEEMIT_INIT_MINER_NAME;
            });

            create_or_modify< account_authority_object, by_account >( *this, account.name, [&]( account_authority_object& a )
            {
            ...
            create_or_modify< witness_object, by_name >( *this, account.name, [&]( witness_object& w )
            {
                w.owner        = account.name;
                w.signing_key  = account.keys.memo_key;
                w.schedule = witness_object::miner;
            });
        }
        // Enable this if you want to validate values
        // validate_invariants();
    }
    FC_CAPTURE_AND_RETHROW();
}
