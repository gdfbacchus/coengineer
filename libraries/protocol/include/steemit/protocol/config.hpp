/*
 * Copyright (c) 2016 Steemit, Inc., Coengineers and contributors.
 */
#pragma once
...
#define STEEMIT_INIT_PUBLIC_KEY_STR             "COE6LLegbAgLAy28EHrffBVuANFWcFgmqRMW13wBmTExqFE9SCkg4"
#define BLOCKCHAIN_NAME                         "COENGINEER"
...
#define STEEM_SYMBOL  (uint64_t(3) | (uint64_t('F') << 8) | (uint64_t('O') << 16) | (uint64_t('R') << 24) | (uint64_t('C') << 32) | (uint64_t('E') << 40)) ///< FORCE with 3 digits of precision (currently only used for backend purposes)
#define SBD_SYMBOL    (uint64_t(3) | (uint64_t('M') << 8) | (uint64_t('A') << 16) | (uint64_t('S') << 24) | (uint64_t('S') << 32)) ///< MASS 3 digits of precision
#define STMD_SYMBOL   (uint64_t(3) | (uint64_t('M') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('D') << 32) ) ///< STEEM Dollars with 3 digits of precision
#define STEEMIT_SYMBOL                          "COENGINEER"
#define STEEMIT_ADDRESS_PREFIX                  "COE"
...
#define STEEMIT_FREE_TRANSACTIONS_WITH_NEW_ACCOUNT 10000
...
#define STEEMIT_INIT_SUPPLY                     int64_t(1002200000.000)
